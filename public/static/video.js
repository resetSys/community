 ...
 * 获取语音对讲通道
 */
function handleGetAudioInfo() {
  //是否选中窗口
  //获取当前选中窗口的ip
  var winInfo = WebVideoCtrl.I_GetWindowStatus(currWin);
  // winInfo.szIP
  if (winInfo != null) {
    I_GetAudioInfo(winInfo.szIP, {
      success: function (xml) {
        showLog("获取语音通道成功", true);
        var AudioChannels = $(xml).find("TwoWayAudioChannel");
        $("col-audioChannel").empty();//清空select选项
        $.each(AudioChannels, function () {
          var id = $(this).find("id").eq(0).text();
          $("col-audioChannel").append("<option value='" + id + "'>" + id + "</option>");
        });
      },
      error: function () {
        showLog("获取语音通道失败", false);
      }
    });
  } else {
    alert("请选择对讲视频");
...
function handleStream() {
  ...
  if (oWndInfo != null) {
    ...
    startPreview(szIp, parseInt(type, 10));
  }
 ...
 * 开始预览
 * @param {预览设备ip} szIP 
 * @param {码流类型} streamType 
 */
function startPreview(szIP, streamType) {
  I_StartRealPlay(szIP, {
    iWndIndex: currWin,//播放窗口
    iStreamType: streamType,//码流类型 1-主码流，2-子码流，默认使用主码流预览
 ...
 * @param {操作类型} index 
 */
var g_bPTZAuto = false;//是否开启自动
function handleStartPtz(index) {
  var winInfo = WebVideoCtrl.I_GetWindowStatus(currWin);
  if (winInfo == null) {
    alert("请选择操作窗口");
    return;
  ...
  var iPTZSpeed = 4;
  if (9 == index && g_bPTZAuto) {
    iPTZSpeed = 0;// 自动开启后，速度置为0可以关闭自动
    showLog("关闭自动",true);
  } else if (9 == index && !g_bPTZAuto) {
    showLog("开启自动",true);
  }

  WebVideoCtrl.I_PTZControl(index, false, {
    iPTZSpeed: iPTZSpeed,
    success: function (xml) {
      console.log(xml);
      showLog("云台操作成功",true);
...
function handleStartTalk() {
  ...
  //获取通道
  var winInfo = WebVideoCtrl.I_GetWindowStatus(currWin);
  var audioChannel = parseInt($("#col-audioChannel>option:checked").val(), 10);
  if (winInfo == null) {
    alert("请选择对讲窗口");
    return;
  }
  ...
  var status = I_StartVoiceTalk(winInfo.szIp, audioChannel);
  if (status == 1) {
    showLog("开始语音对讲...", true);
  } else {
    showLog("开启语音失败", false);
  }
}
...
function handleStopTalk() {
  var status = I_StopVoiceTalk();
  if (status == 1) {
    showLog("关闭语音对讲", true);
  } else {
    showLog("关闭语音对讲失败", false);
  }
}
...
function handleRestart() {
  ...
  WebVideoCtrl.I_Restart(szIp,{
    ...
    error:function (status, xmlDoc) {
      showLog("设备重启失败"+status+xmlDoc,false);
    }
  });
...
function reconnect(szIp) {
  WebVideoCtrl.I_Reconnect(szIp,{
    ...
    error:function (statue,xml) {
      showLog("尝试重新连接设备",false);
    }
  })
}
